...
class Global_utils:
    def download_niconico_comment(logger, additional_info, title_name, episode_title, episode_number, config, title_name_logger, service_type="SERVICE_NAME_HERE"):
        ...
        if additional_info[2]:        
            ...
            def get_niconico_info(stage, data):
                if stage == 1:
                    ...
                    result = requests.post(f"https://public.nvcomment.nicovideo.jp/v1/threads", data=json.dumps(payload), headers=headers).json()
                    return result
