...
function get_x(input) {
    var Fr = br("123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz");
    return Fr.d(input);
}
function get_y(l, e, g) {
    var temp = Cr(l,e,g)
    return temp
}
function decrypt_key(original_json, O, y, x) {
    console.log(O, y)
    console.log(x)
    t = CryptoJS.enc.Hex.parse(O)["words"]
    e = CryptoJS.lib.WordArray.create(new Uint8Array(y))["words"]
    n = CryptoJS.lib.WordArray.create(new Uint8Array(x))["words"]
    ...
    original_json.keys[0].k = decrypted.toString(CryptoJS.enc.Utf8);
    original_json.keys[0].alg = "A128KW"
    return original_json
}
module.exports = { get_x, get_y, decrypt_key };