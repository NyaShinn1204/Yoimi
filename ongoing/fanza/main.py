...
class Dmm_TV_downloader:
    ...
    def authorize(self, email, password):
        ...
        try:
            login_recaptcha_token = Fanza_TV_utils.recaptcha_v3_bypass("https://www.google.com/recaptcha/enterprise/anchor?ar=1&k=6LfZLQEVAAAAAC-8pKwFNuzVoJW4tfUCghBX_7ZE&co=aHR0cHM6Ly9hY2NvdW50cy5kbW0uY29tOjQ0Mw..&hl=ja&v=pPK749sccDmVW_9DSeTMVvh2&size=invisible&cb=nswb324ozwnh")
            ...
            redirect_auth_url = self.session.get("https://www.dmm.com/my/-/authorize/", allow_redirects=False, params=querystring, headers=headers, cookies={"dmm_app": 1}).headers.get("Location")
            ...
            _auth = {
                ...
                "code": redirect_auth_url.replace("dmmmovieplayer://android/auth?code=", ""),
                "redirect_uri": "dmmmovieplayer://android/auth"
            }
    ...
    def get_all_buyed_item(self):
        res = self.session.get("https://video.digapi.dmm.com/purchased/list/all?latest_mylibrary_id=0&limit=50")
        if res.status_code == 200:
    ...
    def get_info(self, mylibrary_id, content_id, product_id):
        ...
        description_req = requests.get(f"https://www.dmm.co.jp/digital/videoa/-/detail/=/cid={content_id}/", cookies={'age_check_done': '1'})
        if description_req.status_code == 404:
            description = None
        else:
            script_tag = BeautifulSoup(description_req.context, "lxml").find("script", {"type": "application/ld+json"})
        ...
        payload_json = '{"exploit_id":"uid:'+str(user_id)+'","mylibrary_id":'+str(mylibrary_id)+',"product_id":"'+str(product_id)+'","shop_name":"videoa","device":"android","HTTP_SMARTPHONE_APP":"DMM-APP","message":"Digital_Api_Mylibrary.getDetail"}' 
        print(payload_json)
        payload = {
            "appid": "android_movieplayer_app",
            "authkey": hmac.new(_AUTHKEY_SECRET.encode('utf-8'), payload_json.encode('utf-8'), hashlib.sha256).hexdigest(),
...
for single_ep in message["list"]:
    #print(single_ep)
    status, episode_info = test.get_info(single_ep["mylibrary_id"], single_ep["content_id"], single_ep["product_id"])
    #print(status)
