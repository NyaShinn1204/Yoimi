# util/unext/utils/license/get_license.py
...
def get_license_unext(pssh, playtoken):
    ...
    response = requests.post(f"{unext_instance.unext_url_list()["runtimeConfig"]["WIDEVINE_PROXY_URL"]}/proxy?play_token={playtoken}", data=challenge)
    print(response.text)
...
def license_vd_ad(video_pssh, audio_pssh, playtoken):
    ...
    response_video = unext_instance.session.post(f"{unext_instance.unext_url_list()["runtimeConfig"]["WIDEVINE_PROXY_URL"]}/proxy?play_token={playtoken}", data=challenge_video)    
    response_video.raise_for_status()
    response_audio = unext_instance.session.post(f"{unext_instance.unext_url_list()["runtimeConfig"]["WIDEVINE_PROXY_URL"]}/proxy?play_token={playtoken}", data=challenge_audio)    
