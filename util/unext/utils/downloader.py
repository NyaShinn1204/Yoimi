...
from util.unext.license.decrypt import *
import util.unext.utils.analyze as analyze_utils
import util.unext.utils.episode_utils as episode_utils
...
def download_video_main(episode_meta_json, value, original_response, episode_license, episode_about, except_resource):
    ...
    if except_resource != None:
        ...
        meta_episodes = analyze_utils.get_video_episode_meta(episode_meta_json["id"])
    ...
    video_downloaded = episode_utils.aria2c(video_info['base_url'], encrypte_video_filename.replace(":", ""))
...
def download_audio_main(episode_meta_json, value, original_response, episode_license, episode_about, except_resource):
    ...
    audio_downloaded = episode_utils.aria2c(audio_info['base_url'], encrypte_audio_filename.replace(":", ""))
...
def download_episode_main(episode_meta_json, value_video, value_audio, episode_license, episode_about, original_response):
    import data.setting as setting
    except_resource = analyze_utils.get_video_episode_meta(episode_meta_json["id"])
    ...
    episode_utils.compile_mp4(video_downloaded.replace("_encrypted", ""),audio_downloaded.replace("_encrypted", ""),f"{original_response['titleName']}_{except_resource["subTitle"].replace(" ", "_")}.mp4",original_response['titleName'])
