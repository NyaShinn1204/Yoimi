...
def check_cookie(load_cookie):
    ...
    #print(load_cookie)
    try:    
        test_cookie = requests.post(setting.unext_url_list()["runtimeConfig"]["COMMAND_CENTER"], json=check_json, cookies=load_cookie)
...
def compile_mp4(video_file, audio_file, output_file, title_name):
    ...
    output_directory = os.path.join(setting.folders["output"], title_name)
    
    if not os.path.exists(output_directory):
        os.makedirs(output_directory, exist_ok=True)

    compile_command = [
        os.path.join(setting.folders["binaries"], "ffmpeg.exe"),
        "-i",
        video_file,
        "-i",
        audio_file,
...
def aria2c(url, output_file_name):
    ...
    aria2c = os.path.join(setting.folders["binaries"], "aria2c.exe")
    aria2c_command = [
        aria2c,
        url,
        "-d",
        os.path.join(setting.folders["temp"], "content", setting.unixtime),
        "-j16",
        "-o", output_file_name,
        "-s16",
        "-x16",
    ...
    setting.downloader_downloader.set("Aria2c")

    # 別スレッドで進行状況を更新
    threading.Thread(target=update_progress, args=(setting.downloader_total, setting.downloader_speed, setting.downloader_downloaded, setting.downloader_elapsed, process), daemon=True).start()

    process.wait()  # ダウンロードの終了を待つ

    # プロセスが終了したらファイルパスを返す
    return os.path.join(setting.folders["temp"], "content", setting.unixtime, output_file_name)
