...
class global_parser:
    ...
    def determine_mpd_type(self, mpd_text: str) -> str:
        ...
        Returns:
            'segment' - セグメント型（SegmentTemplate, SegmentList, SegmentBase のいずれかを含む）
        ...
        try:
            ...
            return None
    def is_have_timeline(self, mpd_text: str):
        times_dict = self.get_segment_times_from_mpd(mpd_text)
        if times_dict is None:
            return False, None
        else:
            return True, times_dict
    def _expand_segment_timeline(self, seg_timeline_el) -> List[int]:
        ...
        """
        NS = {"mpd": "urn:mpeg:dash:schema:mpd:2011"}
        s_list = list(seg_timeline_el.findall("mpd:S", NS))
        times: List[int] = []
        ...
        for idx, s in enumerate(s_list):
            ...
            if d_attr is None:
                raise ValueError("<S> に d がnothing bruh")
            ...
            if r == -1:
                # 次の t まで d 間隔で埋める（次の t が無ければ 1 個 only）
                if next_t is None:
                    # Period 長が分かればそこまでだけど、ここでは 1 個だけ出す
    ...
    def get_segment_times_from_mpd(self, mpd_content: str) -> Optional[Dict[str, List[int]]]:
        ...
        for adapt in root.findall(".//mpd:AdaptationSet", NS):
            ...
            # SegmentTemplate は AdaptationSet または Representation ありそうなのでそこもチェック
            seg_tmpl = adapt.find("mpd:SegmentTemplate", NS)
            if seg_tmpl is None:
                # Representation 側をスキャン
                rep = adapt.find("mpd:Representation/mpd:SegmentTemplate", NS)
                seg_tmpl = rep
