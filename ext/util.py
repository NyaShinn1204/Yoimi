...
def download_command(input: str, command_list: Iterator):
    try:
        ...
        ### check dependence found
        return_dependence = other_util.dependence_check(loaded_config)
        if not return_dependence["shaka_path"] and not return_dependence["mp4_path"] \
           and not return_dependence["aria2c_path"] and not return_dependence["n_m3u8dl_path"]:
            binary_folder = loaded_config["directories"]["Binaries"]
            yoimi_logger.error("Required binaries not found. Please check.")
        ...
        def single_dl(input, video_info = None, season_title = None):
            ...
            elif video_info["content_type"] in ["live", "offline"]:
                output_titlename = video_info["output_titlename"]
            else:
            ...
            if video_info["content_type"] != "offline":
                ...
                if enable_verbose:
                    ...
                    n_m3u8dl_re_command.extend(["--no-log"])
                ...
                yoimi_logger.info("Parsing Offline content file")

                # vrであればここでファイルからの処理を行う。
            ...
            if dl_type == "n_m3u8dl":
                ...
                if service_config["is_drm"] or service_config["is_drm"] == "both":
                    ...
                    if dl_item_list == [] and deliviery_type == "hls" and transformed_data["pssh_list"] == {}:
                        video_decrypt_output = os.path.join(temp_folder_path, "download_decrypt_content.ts")
